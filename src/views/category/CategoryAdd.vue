  ...
  >
    <template #default="{ hide }">
      ...
      <b-form class="p-2" @submit.prevent="sendForm" @reset.prevent="resetForm">
        <!-- Full Name -->
        <b-form-group label="Kategori Adı" label-for="full-name">
          <b-form-input
            id="full-name"
        ...
        <b-form-group label="Kategori Rengi" label-for="full-name">
          <v-select
            v-model="selected"
            label="text"
            :options="options"
            placeholder="Kategori Rengi"
          />
        </b-form-group>
        <!-- Form Actions -->
  ...
  methods: {
    ...
    async sendForm() {
      ...
      try {
        ...
        var addCategoryResponse = await this.$http.post(
          "Categories/create-category",
          this.request
        );

        this.$emit("update:is-add-new-category-sidebar-active", false);
        this.$emit("add-new-row", addCategoryResponse.data);
        this.$emit("show-loading", false);
        this.$swal({
          icon: "success",
          title: "Başarılı!",
          text: `Kategori ekleme işlemi başarılı.`,
          confirmButtonText: "Kapat",
          customClass: {
